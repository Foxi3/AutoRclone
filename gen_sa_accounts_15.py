...
def _create_remaining_accounts(iam,project):
    ...
    while sa_count != 15:
        _create_accounts(iam,project,15 - sa_count)
        sa_count = len(_list_sas(iam,project))
...
def _list_sas(iam,project):
    resp = iam.projects().serviceAccounts().list(name='projects/' + project,pageSize=15).execute()
    if 'accounts' in resp:
...
def _create_sa_keys(iam,projects,path):
    ...
    for i in projects:
        ...
        while current_key_dump is None or len(current_key_dump) != 15:
